...
const imgElem = document.getElementById("web-tech-image");
...
let updateImg = () => {
  imgElem.src = WEB_TECH_IMAGES[indexCurr];
};

nextButton.addEventListener("click", () => {
  indexCurr++;
  if (indexCurr >= WEB_TECH_IMAGES.length) {
    indexCurr = 0;
  }
  updateImg();
});
...
prevButton.addEventListener("click", () => {
  indexCurr--;
  if (indexCurr < 0) {
    indexCurr = WEB_TECH_IMAGES.length - 1;
  }
  updateImg();
});
